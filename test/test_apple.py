...
def investigate_apple_podcast():
    ...
    with open('debug/apple_podcast.html', 'w', encoding='utf-8') as f:
        f.write(response.text)
    logger.info("Saved raw HTML to debug/apple_podcast.html")
    ...
    with open('debug/apple_scripts.txt', 'w', encoding='utf-8') as f:
        for i, script in enumerate(scripts):
            f.write(f"\n=== Script {i} ===\n")
            f.write(script.string if script.string else str(script))
    logger.info("Saved script contents to debug/apple_scripts.txt")
